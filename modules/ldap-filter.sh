...
if [ ${?} -ne 0 ] ; then
	...
	log "token.cgi: meeting_name='${room}', meeting_hash='', email='${auth_mail}', role='owner', auth_uid='${auth_uid}', check_module='ldap-filter.sh': DENIED (not editor)"
	http_403 "Le filtre LDAP ne vous autorise pas à démarrer une réunion"
fi

log "token.cgi: meeting_name='${room}', meeting_hash='', email='${auth_mail}', role='owner', auth_uid='${auth_uid}', check_module='ldap-filter.sh': ok"
